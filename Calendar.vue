<template>
    <div class="container vue-calendar">
        <div class="row">
            <div class="col">
                <!-- Calendar Header -->
                <div v-if="currentView === 'month'" class="btn-group btn-group mb-3" role="group" aria-label="...">
                    <button class="btn btn-primary" @click="prevMonth">
                        < </button>
                            <button class="btn btn-outline-primary">{{ monthName }} {{ currentYear }}</button>
                            <button class="btn btn-primary" @click="nextMonth">></button>
                </div>
                <div class="float-end btn-group btn-group mb-3" role="group" aria-label="...">
                        ...
                        <div v-if="day.events.length" class="mt-2">
                            <div v-for="(event, index) in sortEventsByStartTime(day.events)" :key="index"
                                ...
                                <div v-if="!event.allday && event.start" class="event-badge-time">
                                    {{ new Date(event.start).getHours() }}:{{ new
                                        Date(event.start).getMinutes().toString().padStart(2, '0') }}
                                </div>
                ...
                <div v-for="day in currentWeek" :key="day.date" class="col p-0 border-start">
                    <div v-for="hour in hours" :key="hour" class="hour-slot">
                        ...
                        <div v-for="(event, index) in getEventsInHour(day, hour, false)" :key="index">
                            <template
                                v-if="event && event.start && !isNaN(new Date(event.start).getTime()) && new Date(event.start).getHours() === hour">

                                <!-- Calculate overlapping events and their positions -->
                                <div :style="getEventStyle(event, getOverlappingEvents(day, event).length, getOverlappingEvents(day, event).indexOf(event))"
                                    class="event-badge-day badge bg-primary pointer" @click="onEventClick(event.id)">
                                    <div class="text-start event-badge-title">{{ event.name }}</div>
                                    <div class="text-start mt-2">{{ new Date(event.start).getHours() }}:{{ new
                                        Date(event.start).getMinutes().toString().padStart(2, '0') }}
                                        - {{ new Date(event.end).getHours() }}:{{ new
...
const getEventStyle = (event, overlappingCount = 1, positionIndex = 0, hourHeightPx = 60) => {
    ...
    const startHour = eventStartDate.getHours() + eventStartDate.getMinutes() / 60;
...
const weeksInMonth = computed(() => {
    ...
    for (let i = 0; i < leadingDays; i++) {
        days.push({
            date: new Date(currentYear.value, currentMonth.value, i - leadingDays + 1),
    ...
    for (let i = 1; i <= endOfMonth.getDate(); i++) {
        ...
        const dayEvents = props.events.filter(event => {
            const eventStartDate = new Date(event.start);
            const eventEndDate = new Date(event.end);
            return isSameDay(dayDate, eventStartDate) || (eventStartDate <= dayDate && dayDate <= eventEndDate);
        });
        days.push({
            date: dayDate,
            isCurrentMonth: true,
...
const getEventsInHour = (day, hour, getalldayevent) => {
    return day.events.filter(event => {
        ...
        if (!event.allday) {
            ...
            return eventStartDate.getHours() <= hour && eventEndDate.getHours() >= hour;
        }
